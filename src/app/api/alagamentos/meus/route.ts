...
export async function GET(request: NextRequest) {
  try {
    ...
    // Endpoint específico para buscar alagamentos do usuário logado
    const url = `${API_BASE_URL}/alagamentos/usuario`

    console.log("Buscando alagamentos do usuário logado:", url)
      ...
      if (response.ok) {
        ...
        try {
          ...
          if (contentType && contentType.includes("application/json")) {
            ...
            console.log("Alagamentos do usuário recebidos:", data)
          } else {
    ...
    console.error("Erro geral na API /alagamentos/meus:", error)
    return NextResponse.json({ message: "Erro interno do servidor" }, { status: 500 })
  }
}
